...
function Start(){
	...
	$('#text').text('');
		...
		O1.m = Math.floor(Math.random() * (O1.n - 1)) + 2;
		O2.n = Math.floor(Math.random() * (4)) + 2;
		O2.m = Math.floor(Math.random() * (O1.m - 1)) + 1;
		O2.n = O1.n;
...
function vieti(){
	for(var i = 0; i < 10; i++){
		if(v[i] == '1')
			$('.v:eq('+i+')').css('background-image', 'url("images_lvl1/corect.png")');
			//$('.v:eq('+i+')').css("background-color","green");
			else if(v[i] == '-1')
				$('.v:eq('+i+')').css('background-image', 'url("images_lvl1/gresit.png")');
